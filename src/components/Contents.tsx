...
const Contents: FC<ContentsProps> = ({ blocks }) => {
  ...
  const NotionToHtml = async (blocks: BlockType[]) => {
    ...
    const NotionBlockToTag = (block: BlockType) => {
      ...
      if (block.type === 'video') {
        return `<div class='my-6'><iframe class='w-full youtube-16-9' src="${block.video?.external?.url}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>`
      }
      if (block.type === 'embed') {
        return `<div class='my-6'>
      <iframe style="border-radius:12px" src="${block.embed?.url}" width="100%" youtube-16-9  frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"></iframe>
      </div>`
      }
  ...
  return (
    <div>
      ...
      <section className="text-gray-800">{html}</section>
    </div>
  );
};
